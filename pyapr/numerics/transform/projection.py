...
def maximum_projection(apr: pyapr.APR,
    ...
    if dim == 0:
        return np.array(pyapr.numerics.transform.max_projection_y(apr, parts), copy=False).squeeze()
    elif dim == 1:
        return np.array(pyapr.numerics.transform.max_projection_x(apr, parts), copy=False).squeeze()
    else:
        return np.array(pyapr.numerics.transform.max_projection_z(apr, parts), copy=False).squeeze()


def maximum_projection_alt(apr: pyapr.APR,
                           parts: (pyapr.ShortParticles, pyapr.FloatParticles),
                           dim: int):
    """
    Compute the maximum intensity projection along an axis.
    ...
    if dim == 0:
        return np.array(pyapr.numerics.transform.max_projection_y_alt(apr, parts), copy=False).squeeze()
    elif dim == 1:
        return np.array(pyapr.numerics.transform.max_projection_x_alt(apr, parts), copy=False).squeeze()
    else:
        return np.array(pyapr.numerics.transform.max_projection_z_alt(apr, parts), copy=False).squeeze()
