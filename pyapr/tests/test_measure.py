...
def test_measures(mask_type):
    ...
    assert obj_centers.shape == obj_centers_weighted.shape == (2, 3)

    # check that object centers are within the bounding boxes computed by `find_objects`
    for i in range(obj_centers.shape[0]):
        for j in range(3):
            assert min_coords[i+1, j] < obj_centers[i, j] < max_coords[i+1, j]
            assert min_coords[i+1, j] < obj_centers_weighted[i, j] < max_coords[i+1, j]
